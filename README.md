...
* [`src/cljs/fluxo/`](src/cljs/fluxo/): SPA source files (ClojureScript,
[re-frame](https://github.com/Day8/re-frame))
  - [`core.cljs`](src/cljs/fluxo/core.cljs): contains the SPA entry point, `init`
* [`test/cljs/fluxo/`](test/cljs/fluxo/): test files (ClojureScript,
...
(headless test environment)
    * For Chromium, set the `CHROME_BIN` environment variable in your shell to the command that
    launches Chromium. For example, in Ubuntu, add the following line to your `.bashrc`:
       ...
       ```
7. Clone this repo and open a terminal in the `fluxo` project root directory
...
The `debug?` variable in [`config.cljs`](src/cljs/fluxo/config.cljs) defaults to `true` in
...
```clj
(ns fluxo.example
  (:require [fluxo.config :as config])
